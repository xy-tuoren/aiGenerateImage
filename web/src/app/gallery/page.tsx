...
function ImageEditDialog(props: {
  ...
  const { open, onClose, initialImages, aspectRatio, messageApi, onSaved } = props;
  const [referenceUrls, setReferenceUrls] = useState<string[]>([]);
  const [uploadedImages, setUploadedImages] = useState<Array<{ dataUrl: string; data: string; mimeType: string }>>([]);
  const [prompt, setPrompt] = useState("");
  const [generating, setGenerating] = useState(false);
  const [saving, setSaving] = useState(false);
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [lastResult, setLastResult] = useState<{ imageBase64: string; mimeType: string } | null>(null);
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      reader.onload = () => {
        ...
        setUploadedImages((prev) => [...prev, { dataUrl, data: base64, mimeType }]);
      };
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        ...
        setMessages((prev) => [...prev, { role: "assistant", text: `错误: ${errMsg}` }]);
      ...
      setMessages((prev) => [...prev, { role: "assistant", text: `错误: ${msg}` }]);
      messageApi.error(msg);
    } finally {
      setGenerating(false);
    }
  }, [prompt, generating, referenceUrls, uploadedImages, aspectRatio, messageApi, messages]);
  ...
  const handleSave = useCallback(
    ...
    [lastResult, saving, initialImages, aspectRatio, messages, referenceUrls, messageApi, onSaved, handleClose]
  );

  const allReferenceImages = useMemo(() => {
    const result: Array<{ type: "url" | "uploaded"; url?: string; dataUrl?: string; idx: number }> = [];
    referenceUrls.forEach((url, idx) => result.push({ type: "url", url, idx }));
    uploadedImages.forEach((img, idx) => result.push({ type: "uploaded", dataUrl: img.dataUrl, idx }));
    return result;
        ...
        >
          ...
          {generating ? (
            <div style={{ display: "flex", justifyContent: "flex-start" }}>
              ...
              >
                <Spin size="small" />
                <span style={{ color: "rgba(0,0,0,0.45)", fontSize: 13 }}>AI 正在生成...</span>
              </div>
            </div>
          ) : null}
          <div ref={chatEndRef} />
