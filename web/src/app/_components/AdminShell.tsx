...
export default function AdminShell({ children, defaultSelectedKey, headerTitle }: AdminShellProps) {
  ...
  const menuItems = useMemo(() => {
    ...
    const perms = me?.permissions && typeof me.permissions === "object" ? me.permissions : {};
    const hasAnyUiPerm = Object.keys(perms).some((k) => String(k || "").startsWith("ui:"));
    return all
      .filter((x) => (hasAnyUiPerm ? can(x.uiPerm) : (can(x.uiPerm) || can(x.apiPerm))))
      .map((x) => ({ key: x.key, icon: x.icon, label: x.label }));
  }, [can, me?.permissions]);

  const uiAccess = useMemo(() => {
    if (!authChecked || !me) return { status: "pending" as const, redirectTo: "" };
    const perms = me?.permissions && typeof me.permissions === "object" ? me.permissions : {};
    const hasAnyUiPerm = Object.keys(perms).some((k) => String(k || "").startsWith("ui:"));
    ...
    const hit = rules.find((r) => p === r.prefix || p.startsWith(`${r.prefix}/`));
