...
export async function POST(req: Request) {
  ...
  const url = normalizeUrl(String((body as any).url || ""));
  if (!url) return Response.json({ ok: false, error: "url 不能为空" }, { status: 400 });

  const db = await getMongoDb();
  const col = db.collection<FavoriteDoc>("gallery_favorites");

  const existing = await col.findOne({ userId: user.userId, url } as any, {
    projection: { _id: 1 } as any
  });
  if (existing?._id) {
    await col.deleteOne({ userId: user.userId, url } as any);
    return Response.json({ ok: true, favorited: false, url });
  }

  await col.insertOne({ userId: user.userId, url, createdAt: new Date() } as any);
  return Response.json({ ok: true, favorited: true, url });
}
